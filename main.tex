...
\begin{multicols}{2}
    \begin{enumerate}\tiny \setlength\itemsep{.2ex}
        \item All players draw 10 event cards (\faicon{reply}) from their stack
        ...
        \item The group then discusses further what else could go wrong based on the chosen card. People award +3 point anyone who makes a good point in discussion. Additional points are given to the person that seemlessly include the contents of different cards into the discussion, these points are based on the quantity of event cards used (+1 point for each extra card).
